...
export default function RadialOrbitalTimeline({
  ...
  // 📱 COMPACT DYNAMIC ORBIT RADIUS (90px on phone < 480px, 140px tablet < 640px, 240px desktop)
  useEffect(() => {
    const handleResize = () => {
  ...
  return (
    <div
      className="w-full min-h-[380px] sm:min-h-[740px] flex flex-col items-center justify-center bg-transparent overflow-hidden py-2 sm:py-4 select-none"
      ref={containerRef}
      onClick={handleContainerClick}
    >
      <div className="relative w-full max-w-5xl h-[360px] sm:h-[680px] flex items-center justify-center">
        <div
        ...
        >
          ...
          {/* ⚙️ PURE WHITE VECTOR GEAR (gear-svgrepo-com.svg) ROTATING AT CENTER (COMPACT MOBILE GEAR SIZE) */}
          <div className="absolute z-20 flex items-center justify-center pointer-events-none">
            ...
            >
              <svg
                viewBox="0 0 32 32"
                className="w-14 h-14 sm:w-44 sm:h-44 text-white fill-current drop-shadow-[0_0_12px_rgba(255,255,255,0.75)]"
          ...
          {timelineData.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                {/* 🔴 CIRCULAR NODE BUTTON WITH SVG ICON (RESPONSIVE SIZE FOR PHONES) */}
                <motion.div
                  ...
                  className={`
                    ...
                    ${
                      isExpanded
                        ? "bg-white text-[#8a1c1c]"
                        : isRelated
                        ? "bg-white/80 text-[#8a1c1c]"
                        : "bg-[#8a1c1c] text-white"
                    }
                    border-2 
                    ${
                      isExpanded
                        ? "border-white shadow-xl shadow-white/50 scale-125"
                        : isRelated
                        ? "border-white animate-pulse"
                        : "border-white/70 shadow-md shadow-black/40"
                    }
                ...
                {/* Node Label Title Below Button (Responsive two-line wrapping on mobile to prevent sideways chopping) */}
                <div
                  className={`
                    absolute top-9 sm:top-14 left-1/2 -translate-x-1/2
                    max-w-[85px] sm:max-w-none text-center leading-tight whitespace-normal sm:whitespace-nowrap
                    font-roboto-mono text-[10px] sm:text-base font-extrabold tracking-wide
                ...
                {/* ⚡ INSTANTLY OPENING EXPANDABLE CARD THAT GLIDES TO TOP-CENTER (RESPONSIVE PHONES) */}
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div
                      ...
                      className="absolute top-16 sm:top-24 left-1/2 -translate-x-1/2 w-[85vw] max-w-[320px] sm:w-[480px] md:w-[560px] z-[250] pointer-events-auto transform-gpu will-change-transform"
                    >
                      {/* Responsive Empty White Glassmorphic Card */}
                      <div className="relative bg-white/60 backdrop-blur-2xl border-2 border-white shadow-2xl shadow-black/40 rounded-3xl p-5 sm:p-10 min-h-[160px] sm:min-h-[320px] text-slate-900 flex flex-col items-center justify-center overflow-hidden">
